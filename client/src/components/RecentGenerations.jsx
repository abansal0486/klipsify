      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        {items.map((item, index) => {
          ...
          return (
            <div
              key={item._id || index}
              className="group relative aspect-video rounded-2xl overflow-hidden bg-zinc-900 border border-white/10 shadow-lg hover:shadow-purple-500/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5"
            >
              ...
              {isProcessing ? (
                ...
                <>
                  {/* ── MEDIA ── */}
                  {isVideo ? (
                    <video src={mediaUrl} className="absolute inset-0 w-full h-full object-cover" muted preload="metadata" />
                  ) : (
                    <img src={mediaUrl} alt="media" className="absolute inset-0 w-full h-full object-cover" />
                  )}
